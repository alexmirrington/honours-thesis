...
\begin{figure}[htbp]
    ...
    \caption{An overview of the output module, as implemented in the official compositional attention network source code.}
    \label{fig:output_module}
\end{figure}

\citeauthor{hudson2018compositional}'s description of the original compositional attention network output module is somewhat sparse, so I provide a more detailed description of the ouput module used in the official source code in \figureautorefname{ \ref{fig:output_module}}. Notable inclusions over the output module presented in the original publication are the two dropout layers \(D_0\) and \(D_1\), as well as an additional question projection parameterised by \(W_p\) and \(b_p\) prior to the first dropout layer.
