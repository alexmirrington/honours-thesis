...
{\color{red}Partly chose compositional attention network since it is data-efficient, consequently reasonable quick to train and is well-established as a baseline model on the GQA dataset. Moreover, the compositional attention network is readily extended to any task that requires performing reasoning steps on an input signal, making it a prime candidate for replacing traditional CNN or Faster-RCNN features with scene graph embeddings.}

Inputs:

A knowledge-base of contextual scene graph features \(K_{R_{[i:i+k]}} \in \R^{k \times m \times d_{K_r}}\), where \(m = \max_{\mathcal{G}_r \in \mathcal{G}_{R_{[i:i+k]}}} |\mathcal{V}_r|\) is the number of nodes in the largest graph in the batch.
A holistic question embedding \(\mathcal{Q}_{Q_{[i:i+k]}} \in \R^{k \times d_{K_q}}\)
A knowledge base of contextual question words \(K_{Q_{[i:i+k]}} \in \R^{k \times l \times d_{K_q}}\), where \(l\) is the length of the longest question in the batch.
