...
Moving our focus to \tableautorefname{ \ref{table:question_ablation_structural}}, we see that the BiLSTM question module outperforms other question processing module types for most structural types, with the exception of \textit{compare} type questions, where the GAT question module outperforms other question types by anywhere from 12.58\% to 25.36\%. Whilst it may be tempting to attribute the large decrease in performance for most question modules on compare-type questions to the fact that it is the least common structural question type at only 3.1\% of samples, we cannot ignore that the GAT question module performs so well. Referring forward to the scene graph ablation results in \tableautorefname{ \ref{table:scene_graph_ablation_structural}}, we similarly see that one type of scene graph module outperforms the others for compare-type questions by a significant margin. Interestingly, the outlier in the scene graph ablations is a model that uses BiLSTMs for both question and scene graph processing modules. The common thread here is that when the question module and the scene graph module use the same architecture, we see a large increase in accuracy on question requiring comparative reasoning. Referring back to \tableautorefname{ \ref{table:gqa_question_samples}} in the previous chapter, we see that comparison questions can be either binary or open, and most often ask about the common properties of two objects. After inspecting the control unit attention weights for various question module types, we see that the GAT question module attends to the objects in the question and comparative words like \textit{same} or \textit{different}, where the other question modules focus on words like \textit{do} or \textit{is}, indicating they are doing no more than relying purely on question information to answer questions in these cases. Given the similar pattern we see for scene graph ablations, it is likely that the model struggles to perform well for comparison based questions unless the visual and textual signal are semantically similar, \textit{i.e.} they are processed similarly. I believe this is due to the dot-product attention interaction between the control and visual knowledge-base vectors in the reasoning module. Since dot-product attentions act as similarity measures, it makes sense that the more similar the semantic space between the textual and visual signal, the better the model is able to perform reasoning operations.
