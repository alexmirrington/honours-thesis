...
In this dissertation, I focus solely on visual question answering (VQA), a multimodal task that requires a model to provide an answer to a question about the contents of an image. More formally, given a hypothesis class \(H\), we aim to learn the optimal hypothesis \(h \in H\) for a given set of training data a VQA model aims to learn a function \(f(q, i) \rightarrow a\) where \(q \in Q\), \(i \in I\), \(a \in A\) for a set of questions, images and answers \(Q\), \(I\) and \(A\) respectively. 
