...
\nomenclature[D, 08]{\(R_{[i:i+k]} = \{r_j \mid ((q_j, r_j), y_j) \in S\}_{j=i}^{\min(i + k, n)}\)}{A batch of visual representations of size \(\min(k, n - i)\) from a dataset \(S\) of size \(n\), starting at index \(i\).}

\nomenclature[D]{\(H\)}{A matrix of embedded features.}
